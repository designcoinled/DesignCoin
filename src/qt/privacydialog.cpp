                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSIGN ought to be enough for anybody." - Bill Gates, 2017
    ui->zSIGNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSIGNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSIGN->setEnabled(false);
        ui->pushButtonMintzSIGN->setToolTip(tr("zSIGN is currently disabled due to maintenance."));

        ui->pushButtonSpendzSIGN->setEnabled(false);
        ui->pushButtonSpendzSIGN->setToolTip(tr("zSIGN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSIGNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSIGN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSIGN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSIGN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSIGN in ") +
...
void PrivacyDialog::on_pushButtonSpendzSIGN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSIGN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSIGN
        sendzSIGN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSIGN
    sendzSIGN();
}

void PrivacyDialog::on_pushButtonZSignControl_clicked()
{
    ZSignControlDialog* zSignControl = new ZSignControlDialog(this);
    zSignControl->setModel(walletModel);
    zSignControl->exec();
}

void PrivacyDialog::setZSignControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSignSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSIGN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid DesignCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSIGNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSIGN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SIGN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSIGN</b>";
    ...
    // use mints from zSign selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSignControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSignControlDialog::GetSelectedMints();
    }
    ...
    // Clear zsign selector in case it was used
    ZSignControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSign Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Sign, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSign Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSIGN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSIGN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSIGN "));
    ui->labelzSIGNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSIGNSyncStatus->setVisible(fShow);
}
