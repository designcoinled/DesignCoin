...
#include "ui_zsigncontroldialog.h"
...
std::list<std::string> ZSignControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSignControlDialog::listMints;

ZSignControlDialog::ZSignControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSignControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSignControlDialog::~ZSignControlDialog()
{
    delete ui;
}

void ZSignControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSignControlDialog::updateList()
{
...
void ZSignControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSignControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSign_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSignControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSignControlDialog::GetSelectedMints()
{
...
void ZSignControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
